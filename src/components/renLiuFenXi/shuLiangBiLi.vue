...
export default {
  ...
  methods: {
    ...
    formatterLabel(newData) {
      ...
      percent = ((newData.value / total) * 100).toFixed(0);
      if (newData.name !== "") {
        return newData.name + "：" + percent + "%";
      } else {
        return "";
      }
    },
  ...
  computed: {
    newData() {
      if (this.data) {
        return this.updateArr(this.data);
      } else {
        return sexData;
      }
    },
  ...
  watch: {
    data() {
      ...
      // this.option.series[0].itemStyle.normal.label.formatter = "{b}: {d}";
    },
  },
  mounted() {
    this.$nextTick(() => {
